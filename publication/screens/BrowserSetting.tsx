...
import {
  ...
  Image,
  TouchableOpacity,
  ActivityIndicator,AppState,Appearance,
  useWindowDimensions
} from "react-native";
...
const BrowserSettingScreen = ({ route, navigation }) => {
  ...
  const [currentUrl, setCurrentUrl] = useState("");const [count, setCount] = useState(0);
   const { width, height } = useWindowDimensions();
   const isLandscapeMode = width > height ? true : false;
  const webviewRef = useRef(null);const appState = useRef(AppState.currentState);const isFocused = useIsFocused();

  const js =title=="privacyPolicy"||title=="contactus"?
            `(function () {
  ...
  React.useEffect(
    () =>
      navigation.addListener("focus", () => {
        ...
        setTheme(global.theme);setJscode(js);
        global.screen="BrowserSetting";global.indicator++;setCount(global.indicator);
      }),
    []
  );
    ...
    useFocusEffect(
            React.useCallback(() => {
              const subscriptionF = AppState.addEventListener("change", handleAppStateChangeF);console.log('subscription removed in BrowserSetting');
                  return () =>{if(subscriptionF)subscriptionF.remove();console.log('subscription removed in BrowserSetting'); }
            }, [])
          );
    const handleAppStateChangeF = nextAppState => {
       console.log('BrowserSetting1 detected state change:',global.screen,'sub:',global.subScreen,'State:',appState);//if(global.screen=="Settings")navigation.navigate("Settings");
       if(global.screen=="BrowserSetting"){
           try{
              if (appState.current.match(/inactive|background/) && nextAppState === 'active') {
                  ...
                  console.log('BrowserSetting2.......App has come to the foreground! scheme:',global.colorScheme,commonBackground(theme));
           ...
           }catch(err){console.log('err in BrowserSetting');return;}
       }
    }
...
export default BrowserSettingScreen;
