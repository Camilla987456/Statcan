 ...
 * 
 * It locks the screen orientation to the orientation passed in the first argument, and unlocks it when
 * the component unmounts
 * @param {OrientationLock} [orientation] - The orientation you want to lock the screen to.
 * @returns An object with a property called currentOrientation.
 * @param orientation optional enum which sets the screen orientation, it can be from the following types: ```
    DEFAULT = 0,
    ALL = 1,
    PORTRAIT = 2,
    PORTRAIT_UP = 3,
...
export function useScreenOrientation(
  ...
  useEffect(() => {
    if (
      !orientation ||
      orientation === ScreenOrientation.OrientationLock.UNKNOWN
    )
      return;
